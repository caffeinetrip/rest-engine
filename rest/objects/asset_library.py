    ...
    def __init__(self, settings, resources=None):
        ...
        for sequence_name, sequence_config in self.specs.get('sequences', {}).items():
            ...
            if len(sequence_parts) == 1:
                sorted_items = [(int(key.split('_')[-1].split('.')[0]) if key.startswith('img_') else int(key.split('.')[0]), value) for key, value in current_dict.items() if isinstance(value, pygame.Surface)]
                sorted_items.sort(key=lambda x: x[0])
                sequence_images = [item[1] for item in sorted_items]
            else:
                for part in sequence_parts:
                    if isinstance(current_dict, dict) and part in current_dict: current_dict = current_dict[part]
                    else: current_dict = None; break
                if current_dict and isinstance(current_dict, dict):
                    sorted_items = [(int(key.split('_')[-1].split('.')[0]), value) for key, value in current_dict.items() if isinstance(value, pygame.Surface)]
                    sorted_items.sort(key=lambda x: x[0])
                    sequence_images = [item[1] for item in sorted_items]
            if sequence_images: self.sequences[sequence_name] = Animation(sequence_images, config=sequence_config)

class AssetLibrary:
    def __init__(self, directory=None):
        self.directory = directory
        self.assets = {}
        if directory: self.initialize(directory)
    ...
    def build_asset_registry(self):
        ...
        for object_dir in os.listdir(self.directory):
            ...
            if not os.path.isdir(object_path): continue
            settings = {
                'uid': object_dir,
                'size': [16, 16],
                'offset': [0, 0],
            ...
            if os.path.exists(config_path):
                ...
                for root, dirs, files in os.walk(object_path):
                    ...
                    if rel_path == '.' or not files: continue
                    png_files = sorted([f for f in files if f.endswith('.png') and (f.startswith('img_') or f[0].isdigit())], key=lambda x: int(x.split('_')[-1].split('.')[0]) if x.startswith('img_') else int(x.split('.')[0]))
                    if png_files:
                        sequence_name = rel_path
                        num_frames = len(png_files)
                        durations = [0.06] * num_frames if 'particles' in object_dir else [0.15] * num_frames if 'idle' in sequence_name else [0.05] * num_frames
                        settings['sequences'][sequence_name] = {'offset': [0, 0], 'rate': 1.0, 'repeat': True, 'frozen': False, 'durations': durations}
                if settings['sequences']:
                    with open(config_path, 'w') as f: json.dump(settings, f, indent=4)
