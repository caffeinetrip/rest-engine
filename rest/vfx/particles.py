...
class Particle:
    def __init__(self, pos, particle_type, velocity=(0, 0), decay_rate=1.0, advance=0.0, behavior='idle', colors=None, z=0, physics_source=None):
        ...
        # КОРОТКОЕ ВРЕМЯ ЖИЗНИ ДЛЯ ПЫЛИ
        self.lifetime = 0.0
        self.max_lifetime = random.uniform(0.3, 0.6)  # Очень короткое время жизни
        self.fade_duration = 0.2  # Быстрое затухание
        self.fade_start_time = self.max_lifetime - self.fade_duration
        self.alpha = 180
        self.should_remove = False
        
        # МАСШТАБ ЧАСТИЦЫ (МАЛЕНЬКИЕ)
        self.scale = random.uniform(0.3, 0.7)  # Маленькие частицы
        
        # НАСТРОЙКА АНИМАЦИИ
        asset = G.asset_library[self.type]
        if not asset:
            logging.error(f"Asset '{self.type}' not found in asset library. Available assets: {list(G.asset_library.keys())}")
            raise KeyError(f"Asset '{self.type}' not found in asset library.")
        if self.type not in asset.sequences:
            logging.error(f"Sequence '{self.type}' not found for asset '{self.type}'. Available sequences: {list(asset.sequences.keys())}")
            raise KeyError(f"Sequence '{self.type}' not found for asset '{self.type}'.")
        
        self.animation = asset.sequences[self.type].copy()
        self.animation.config['loop'] = True
        
        # МЕДЛЕННАЯ АНИМАЦИЯ ДЛЯ ПЫЛИ
        self.animation_speed = random.uniform(0.8, 1.2)  # Не слишком быстро
        
        # НАЧИНАЕМ С ПЕРВОГО КАДРА (БОЛЬШАЯ ЧАСТИЦА)
        ...
        if colors:
            ...
            if colors_id in ANIMATION_CACHE:
                ...
                self.animation.frame_index = 0  # Всегда начинаем с первого кадра
                self.animation.timer = 0
            else:
    ...
    def update(self, dt=None):
        ...
        # Проверяем, нужно ли удалять частицу
        if self.lifetime >= self.max_lifetime:
            self.should_remove = True
            return True
        
        # Обновляем анимацию
        self.animation.update(dt * self.decay_rate * self.animation_speed)
        
        # Расчет альфы для плавного затухания
        if self.lifetime > self.fade_start_time:
            ...
            self.alpha = int(180 * (1.0 - fade_progress))
        else:
            ...
            if self.lifetime < fade_in_duration:
                ...
                self.alpha = int(180 * fade_in_progress)
            else:
                self.alpha = 180
    ...
    def renderz(self, camera_offset=(0, 0), group='default'):
        ...
        # Создаем копию и применяем альфу только если нужно
        if self.alpha < 180:
            img = img.copy()
            img.set_alpha(self.alpha)
